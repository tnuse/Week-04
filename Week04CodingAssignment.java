...
// 12. Write a method called willBuyDrink that takes a boolean isHotOutside, and a double moneyInPocket, and returns true if it is hot outside and if moneyInPocket is greater than 10.50.
	...
	// Create boolean method to return true if isHotOutside is true and moneyInPocket is greater than 10.50, otherwise return false.
...
// 13. Create a method of your own that solves a problem. In comments, write what the method does and why you created it.
	...
	// Create Method to calculate the total grocery bill by using elements in groceryList List to loop through keys in the groceryCost Map and adding the values of related keys to calculate the sum.  
	public static double groceryCalculator(Map<String, Double> groceryCost, List<String> groceryList) {
		
		// Create groceryTotal double variable to store the sum of all values base on keys in groceryCost Map
		double groceryTotal = 0.00;
		
		// Enhanced for Loop to cycle through elements in groceryList List and add value to groceryTotal double for matching key from groceryCost Map. 
		for (String str : groceryList) {
			if (groceryCost.containsKey(str)) {
				groceryTotal += groceryCost.get(str);
			}
		}
		// return groceryTotal double after for Loop completes.
		return groceryTotal;
	}
